...
const CONFIG = {
  ...
  /* Max Green Light */
  AI_MAX_GREEN: 10,      // max green allowed for AI decisions
  BASE_MAX_GREEN: 10,    // optional; baseline max (you can remove this if you want)
...
class TrafficLight {
  constructor(mode = 'baseline') {
    ...
    this.onGreenStart  = null;   // callback: fired when green starts (allred→green)
// callback: fired at yellow→allred
  }
  ...
  tick(dt) {
    ...
    if (this.subPhase === 'green') {
      ...
      this.subPhase = 'green';
          if (this.onGreenStart) this.onGreenStart(this._phaseKey);
}
  }
  ...
  setNextPhase(phaseId, duration) {
    ...
    if (!phKey) return;
    const maxGreen = (this.mode === 'ai')
      ? (CONFIG.AI_MAX_GREEN ?? 10)
      : (CONFIG.BASE_MAX_GREEN ?? 60); // or CONFIG.FIXED_GREEN if you prefer

    const d = Math.max(5, Math.min(maxGreen, Math.round(Number(duration) || CONFIG.FIXED_GREEN)));
...
class Simulation {
  constructor(canvas, mode) {
    ...
    this.aiReason  = 'Awaiting first AI cycle…';
        ...
        this._lastExecutedPhaseId = 0; // tracks phase that actually started green (1-4)

this._aiTimer = 0;   // sim-seconds counter; AI polled every 10 s (spec)
    if (mode === 'ai') {
      // AI fires on phase-end AND on periodic tick
      this.light.onPhaseEnd = (ph) => this._callAI(ph);

      // Record which phase actually starts GREEN (prevents "same lane again" after cleared)
      this.light.onGreenStart = (phKey) => {
        const id = Number(Object.entries(PHASE_NUM_MAP).find(([, v]) => v === phKey)?.[0]) || 0;
        if (id >= 1 && id <= 4) this._lastExecutedPhaseId = id;
      };
    }
  }
  ...
  async _callAI(contextPhase) {
    ...
    try {
      // Plan ONLY during ALLRED or near end of GREEN (<=2s remaining)
      const rem = this.light.remaining();
      const okToPlan =
        (this.light.subPhase === 'allred') ||
        (this.light.subPhase === 'green' && rem <= 2);

      if (!okToPlan) return;

      // Debounce: avoid repeated planning in same phase window
      const planKey = `${this.light.currentPhase}:${this.light.subPhase}:${Math.ceil(rem)}`;
      if (this._lastPlanKey === planKey) return;
      this._lastPlanKey = planKey;

      const q = this.laneQueues();

      // Demand per approach (1-4) — matches what you see on screen
      const demand = {
        1: q.north_left + q.north_straight + q.north_right,
        2: q.east_left  + q.east_straight  + q.east_right,
        3: q.south_left + q.south_straight + q.south_right,
        4: q.west_left  + q.west_straight  + q.west_right,
        5: 0, 6: 0, 7: 0, 8: 0, 9: 0, 10: 0,
      };

      const totalDemand = demand[1] + demand[2] + demand[3] + demand[4];

      // Low traffic = rotate faster to keep movement smooth
      const LOW_TRAFFIC = totalDemand <= 12;
      const LOW_BASE = 6;  // seconds
      const LOW_MAX  = 10; // seconds

      const clamp = (n, lo, hi) => Math.max(lo, Math.min(hi, n));

      // Duration caps (prevents 30/35s)`
      const FIXED_DUR = CONFIG.FIXED_GREEN;
      const AI_MAX = (CONFIG.AI_MAX_GREEN ?? 10);
      const MIN_G = 5;

      // Current phase id (prefer contextPhase, else currentPhase)
      const contextKey = contextPhase || this.light.currentPhase;
      const curId = Number(
        Object.entries(PHASE_NUM_MAP).find(([, v]) => v === contextKey)?.[0]
      ) || 1;

      // Fixed clockwise next: 1→2→3→4→1
      const cw = [1, 2, 3, 4];
      const cur = cw.includes(curId) ? curId : 1;
      const fixedNextId = cw[(cw.indexOf(cur) + 1) % 4];

      // Best demand
      const bestId = [1, 2, 3, 4].reduce((best, id) => (
        demand[id] > demand[best] ? id : best
      ), fixedNextId);

      // Use the phase that ACTUALLY started green (recorded via onGreenStart)
      const lastServed = this._lastExecutedPhaseId || curId;

      // Sorted for 2nd best
      const sorted = [1, 2, 3, 4].slice().sort((a, b) => demand[b] - demand[a]);
      const secondBest = sorted.find(id => id !== bestId) || fixedNextId;

      // HARD no-repeat: if best is the same as last served, force 2nd best
      // unless best is still clearly worse (only allow repeat when traffic is heavy + big gap)
      const REPEAT_ALLOW_GAIN = 4; // must be >=4 cars more to repeat
      let finalBest = bestId;

      if (bestId === lastServed) {
        const gap = (demand[bestId] - demand[secondBest]);
        const allowRepeat = (!LOW_TRAFFIC) && (gap >= REPEAT_ALLOW_GAIN);
        if (!allowRepeat) finalBest = secondBest;
      }

      // Override clockwise if congestion elsewhere is clearly worse
      const fixedScore = Number(demand[fixedNextId] || 0);
      const bestScore  = Number(demand[finalBest] || 0);

      const OVERRIDE_GAIN  = LOW_TRAFFIC ? 1 : 2;
      const OVERRIDE_RATIO = LOW_TRAFFIC ? 1.1 : 1.2;

      const overrideClockwise =
        (finalBest !== fixedNextId) &&
        (
          (bestScore - fixedScore) >= OVERRIDE_GAIN ||
          (fixedScore > 0 && (bestScore / fixedScore) >= OVERRIDE_RATIO)
        );

      const nextId = overrideClockwise ? finalBest : fixedNextId;

      // Duration target
      const loadNext = Number(demand[nextId] || 0);
      let targetDur;

      if (LOW_TRAFFIC) {
        targetDur = clamp(LOW_BASE + Math.round(loadNext * 0.3), MIN_G, LOW_MAX);
      } else {
        const BASE = 10, SCALE = 6;
        targetDur = clamp(Math.round(BASE + Math.sqrt(loadNext) * SCALE), MIN_G, AI_MAX);
      }

      // Early-cut current green when overriding (only if we already served minimum green)
      const maybeEarlyCut = () => {
        const MIN_SERVE = 5;
        if (this.light.subPhase !== 'green') return;

        const ran = this.light.timer;
        const rem2 = this.light.remaining();

        if (ran >= MIN_SERVE && rem2 > 1) {
          this.light.subPhase = 'yellow';
          this.light.timer = 0;
        }
      };

      const modeTag = overrideClockwise
        ? `OVERRIDE(best=${finalBest} vs cw=${fixedNextId})`
        : `CW(next=${fixedNextId})`;

      /* ── heuristic fallback ─────────────────────────────────── */
      const heuristic = (reason) => {
        this.light.setNextPhase(nextId, targetDur);
        if (overrideClockwise) maybeEarlyCut();

        this.aiReason = (reason ||
          `Smart timing → ${modeTag} next=${nextId} dur=${targetDur}s ` +
          `[N:${demand[1]} E:${demand[2]} S:${demand[3]} W:${demand[4]}] ` +
          `lastServed=${lastServed}` +
          (LOW_TRAFFIC ? ' | low-traffic fast-rotate' : '')
        ).slice(0, 250);
      };
      ...
      // Backend call is used mainly for duration suggestion + reason.
      ...
      const res = await fetch(CONFIG.CLOUD_FN_URL, {
        ...
        body: JSON.stringify({
          ...
          chosen_next_phase_id: nextId,

          fixed_green: FIXED_DUR,
      ...
      const aiDur = clamp(Math.round(aiDurRaw || targetDur), MIN_G, AI_MAX);

      const modelImproved = (data && typeof data.improved_over_fixed === 'boolean')
        ? !!data.improved_over_fixed
        : (Math.abs(aiDur - targetDur) <= Math.abs(FIXED_DUR - targetDur));

      if (Number.isFinite(aiDurRaw) && modelImproved) {
        this.light.setNextPhase(nextId, aiDur);
        if (overrideClockwise) maybeEarlyCut();

        this.aiReason = String(
          data.reason || `AI timing accepted → ${modeTag} next=${nextId} dur=${aiDur}s`
        ).slice(0, 250);

      } else if (Number.isFinite(aiDurRaw)) {
        // If overrideClockwise, keep targetDur; else keep fixed duration
        const durWhenRejected = overrideClockwise ? targetDur : FIXED_DUR;

        this.light.setNextPhase(nextId, durWhenRejected);
        if (overrideClockwise) maybeEarlyCut();

        this.aiReason = overrideClockwise
          ? `AI timing rejected, but override active → next=${nextId} dur=${targetDur}s.`
          : `AI timing rejected. Using fixed ${FIXED_DUR}s for next=${nextId}.`;

      } else {
        heuristic(`Bad AI response (duration=${data?.duration}) — heuristic`);
      }

    } catch (err) {
      this.aiReason = `AI error: ${String(err?.message || err).slice(0, 120)} — will re-plan near phase end.`;
    } finally {
      this.aiPending = false;
    }
  }
